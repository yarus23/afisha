...
Ext.ux.BufferedList = Ext.extend(Ext.List, {
    ...
    rebuildTemplate: function () {
        ...
        this.itemTplDelayed = '<tpl for="."><div class="x-list-item ' + this.itemCls + '"' + (this.height ? 'style=\"height:' + this.height + ' width=100% \"' : '') + '><div class="x-list-item-body">' + this.itemTpl + '</div>';
    ...
    renderOnScroll: function (startRecord) { // startRecord optional
        ...
        if( !this.topProxy ) return; // empty list
        ...
        if (Ext.isNumber(startRecord)) {
            ...
            //console.log('listBottomMargin ' + listBottomMargin + ' scrollPos ' + scrollPos + ' thisHeight ' + thisHeight + ' top+content ' + (this.topProxy.getHeight() + this.listContainer.getHeight()));
            // scrolled into "white space"
            //console.log('listTopMargin ', listTopMargin);
            if (listTopMargin < -thisHeight || listBottomMargin > thisHeight) {
                incrementalRender = false;
                scrollDown = true;
                
                if( listBottomMargin >= thisHeight ) {
                    var avgLen = this.listContainer.getHeight() / this.listContainer.dom.childNodes.length;
                    newTop = Math.max(0, maxIndex - Math.floor(thisHeight / avgLen));
                } else
                    newTop = 0;
                newBottom = Math.min(newTop + this.minimumItems - 1, maxIndex);
                //console.log('boom!!! ' + newTop);
            }
        ...
        //console.log('listBottomMargin ' + listBottomMargin);
        ...
        if (this.bottomItemRendered === 0 || !incrementalRender) {
            ...
            //console.log('incrementalRender ', incrementalRender);
            //console.log('this.bottomItemRendered ', this.bottomItemRendered)
            //console.log('replace items top ' + newTop);
            nItems = this.replaceItemList(newTop, this.minimumItems);
            // console.log('top proxy height', this.topProxy.getHeight());
            if( newTop )
              this.scroller.scrollTo({ x: 0, y: this.topProxy.getHeight() });
        }
        // incremental - scrolling down
            ...
            // todo: make compatible removeItems with group headers
            if( !this.useGroupHeaders && this.removeWhileScroll )
              this.removeItems(true, nItems);
        ...
        // zero out bottom proxy if last rendered item is last item in the store
        //console.log('newBottom ' + newBottom + ' maxIndex ' + maxIndex);
        if (newBottom >= maxIndex) {
            
            // set up to bottom proxy before then clear it
        ...
        for (i = nHeaders - 1; i >= 0; i--) {
            ...
            if (groupTop < headerMoveTop) {
                ...
                if (groupTop > scrollPos) {
                    ...
                    this.updateHeaderText(decodeURIComponent(this.getPreviousGroup(headerNode.innerHTML)).toUpperCase());
                }
                else {
    ...
    buildItemHtml: function (firstItem, lastItem) {
        ...
        for (i = firstItem; i <= lastItem; i++) {
            ...
            if (grpHeads) {
                ...
                if ((i === this.groupStartIndex(groupId) && groupId.length) ) {
    ...
    // remove items to make the list scroll faster
    removeItems:function(top, nItems) {
        first = top ? this.topItemRendered : this.bottomItemRendered;
        
        if (first + nItems > this.bottomItemRendered ) {
            nItems = this.bottomItemRendered - first;
        }
     
        var oldHeight = this.listContainer.getHeight();
        // delete nodes
        var len = first + nItems;
        
        var tor = this.all.elements.splice(first - this.topItemRendered, nItems);

        for(var i = 0; i < tor.length; i++)
           Ext.removeNode(tor[i]);
             
       //console.log(nItems, this.all.elements.length);
       //console.log('removed ', nItems);
        // correct proxy height
        if( top ) {
           this.topProxy.setHeight(this.topProxy.getHeight() + oldHeight - this.listContainer.getHeight());
           this.topItemRendered += nItems;
        } else {
           this.bottomProxy.setHeight(this.bottomProxy.getHeight() + oldHeight - this.listContainer.getHeight());
           this.bottomItemRendered -= nItems;
        }
    ...
    initGroupIndexMap: function () {
        ...
        for (i = 0; i < sc; i++) {
            key = encodeURIComponent(store.getGroupString(store.getAt(i)).toLowerCase());
    ...
    getKeyFromId: function (groupId) {
        return encodeURIComponent(groupId.toLowerCase());
    },
    ...
    updateItemList: function () {
        if( !this.listContainer )// wait until refresh
          return;
